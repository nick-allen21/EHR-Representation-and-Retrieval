...
This repository contains a dataset designed for training large language models specialized in retrieving information from discharge summaries taken from electronic health records through context-augmented generation. The data is extracted and structured from the MIMIC-IV-Note database and is available in two formats: JSON and CSV. Here, structure of both files along with a JSON example entry is provided:

## Dataset Files

1. **mimic_note_qa.json**
2. **mimic_note_qa.csv**
3. **mimic_note_qa_verified.json**
4. **mimic_note_qa_verified.csv**
...
```csv
original_id,note_id,extended_context,qa_pairs
placeholder_original_id,placeholder_note_id,placeholder_extended_context_boolean,"[{""question"": ""placeholder_question_text"", ""answer"": ""placeholder_answer_text"", ""correct"": ""placeholder_boolean""}]"
```
